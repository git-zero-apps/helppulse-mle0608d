...
export default function EditSatisfactionRatingPage() {
  ...
  if (!record) {
    return (
      <div className="rounded-lg bg-red-50 border border-red-200 p-4">
        <p className="text-sm text-red-700">Satisfaction Rating not found.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mt-2">Edit Satisfaction Rating</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card space-y-6">
        ...
        <div>
          ...
          <input id="rating" name="rating" type="number" className="input" defaultValue={String(record.rating ?? "")} required />
        </div>
        <div>
          <label htmlFor="feedback" className="label">Feedback</label>
          <input id="feedback" name="feedback" type="text" className="input" defaultValue={String(record.feedback ?? "")} />
        </div>

        <div className="flex items-center gap-3 pt-4 border-t">
          <button type="submit" disabled={loading} className="btn-primary">
            {loading ? "Saving..." : "Update Satisfaction Rating"}
          </button>
