    ...
    <div>
      ...
      {items && items.length > 0 ? (
            ...
            <tbody className="divide-y divide-gray-200">
              {items.map((item: Record<string, unknown>) => (
                <tr key={item.id as string} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-900 whitespace-nowrap">{String(item.total_tickets ?? "—")}</td>
                  <td className="px-6 py-4 text-right text-sm whitespace-nowrap">
                    <Link href={`/dashboard/customers/${item.id}/edit`} className="text-brand-600 hover:text-brand-800 font-medium mr-4">
                      Edit
                    </Link>
                    <DeleteButton id={item.id as string} table="customers" />
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="card text-center py-12">
          ...
          <h3 className="mt-4 text-sm font-semibold text-gray-900">No customers</h3>
          <p className="mt-1 text-sm text-gray-500">Get started by adding your first customer.</p>
          <Link href="/dashboard/customers/new" className="btn-primary mt-6 inline-flex">
            Add Customer
          </Link>
        </div>
      )}
